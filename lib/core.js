...
exports = module.exports = class {
	...
	_handler(event, context, callback) {
		...
		if(!route) return callback(null, new Response(reply.response("Route not found").code(404), this));
		...
		(async ()=>{
			...
			.then(res=>{ log.end(event.path+' [handler]'); callback(null, new Response(res, this)); })
			.catch(err=>{ log.end(event.path+' [handler]'); callback(null, new Response(err, this)); context.fail(err); });

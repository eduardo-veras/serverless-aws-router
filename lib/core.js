...
exports = module.exports = class {
	...
	_handler(event, context, callback) {
		...
		(async ()=>{
			...
			if(!route) return new Response(reply.response("Route not found").code(404), this);
			...
			try {
				...
				const response = new Response(payload, this);
				await this.eventBus.emit('onPreResponse', response);
			...
			.catch(err=>{ callback(null, new Response(err, this)); context.fail(err); });
